fn main() {
    ...
    println!("user_with_simple_code: name: {}, email: {}", user_with_simple_code.username, user_with_simple_code.email);


    let user_build_from_other = build_from_user(user);
    println!("user_build_from_other: name: {}, email: {}", user_build_from_other.username, user_build_from_other.email);
//    println!("user: name: {}, email: {}", user.username, user.email); // 这里会报错，因为user的所有权已经在上面函数中转移到临时变量中了
    ...
    println!("user_get_some_field_from_other: name: {}, email: {}", user_get_some_field_from_other.username, user_get_some_field_from_other.email);
    // 这里因为user_build_from_other的active 实现了copy trait 所以所有权没有转交、email和username没有用到，所有权也没有转交，所以user整体仍然可用
    println!("user_build_from_other: name: {}, active: {}", user_build_from_other.username, user_build_from_other.active);
//    println!("user_build_from_other: email: {}", user_build_from_other.email); // 这里会报错，因为email在build新的user的时候，所有权已经转交了

    let black = Color(0,0,0);
    println!("R: {} G: {} B: {}", black.0, black.1, black.2);

    println!("user_get_some_field_from_other: {:?}", user_get_some_field_from_other);
    dbg!(&user_get_some_field_from_other); // dbg 宏会获取参数所有权

    println!("get username of user_get_some_field_from_other: {}", user_get_some_field_from_other.get_user_name());
...
impl User {
    ...
    fn attach_name(self: &mut Self, subfix: &str) { // &mut self是self: &mut Self这种方式的缩写
